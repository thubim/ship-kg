...
class EntityCombine:
    ...
    def combine(self, words, netags, user_dict):
        """根据命名实体的B-I-E进行词合并
        Args:
        ...
        while i < length:
            if 'B-' in netags[i]:
                ...
                while j < length:
                    ...
                    j += 1
                word_type = "None"
                if newword in user_dict.keys():
                    word_type = user_dict[newword]
                offset = words[i].offset
                words_combine.append(WordUnit(n, newword, self.judge_postag(netags[j-1]), word_type, offset))
                n += 1
                i = j
            else:
                words[i].ID = n
            ...
            i += 1
        return self.combine_comm(words_combine, user_dict)

    def combine_comm(self, words, user_dict):
        """根据词性标注进行普通实体合并
        Args:
        ...
        while i < len(words):
            ...
            if (self.is_entity(word.postag) and self.is_entity(words[i-1].postag) 
                ...
                words_combine.append(WordUnit(n, newword, words[prev_i].postag, words[prev_i].type, words[prev_i].offset))  # 添加上一个词
                n += 1
                newword = word.lemma  # 当前词作为新词
                prev_i = i
            i += 1
        # 添加最后一个词
        words_combine.append(WordUnit(n, newword, words[prev_i].postag, words[prev_i].type, words[prev_i].offset))
        return words_combine
    ...
    def is_entity(self, netag):
        ...
        # 地名，机构名，人名，其他名词，缩略词
        if netag in {'n', 'ns', 'ni', 'nh', 'nz', 'j', 'nl', 'r'}:
            flag = True
        return flag
